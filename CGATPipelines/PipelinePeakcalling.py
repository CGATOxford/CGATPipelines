...
def getPeakShiftFromZinba(infile):
    '''reads are shifted to account for the offset in forward or reverse strand reads, which helps determine most likely bases involved in protein binding. How much to shift is determined by the fragment size generated by library preparation and is stored within the ZINBA output.
...
def summarizeMACSsolo(infiles, outfile):
    ...
    For MACS run without a control file where certain terms are missing from log file.
...
def summarizeMACSFDR(infiles, outfile):
    '''compile table with the number of peaks that would remain after filtering using different FDR thresholds.
...
def runZinba(infile,
    ...
    tag_size : integer
        length of aligned reads/tags; determined by calculating average region length of input file [default=50]
...
def loadZinba(infile, outfile, bamfile,
    ...
    This method loads only positive peaks into a tab separated value (tsv) text files. It then filters peaks by p-value, q-value and fold change and loads the diagnostic data and re-calculates peakcenter, peakval, etc. using the supplied bamfile. This method uses the refined peak locations. Zinba peaks can be overlapping. This method does not merge overlapping intervals. Zinba calls peaks in regions where there are many reads inside the control. Thus this method applies a filtering step removing all intervals in which there is a peak of more than readlength / 2 height in the control.
    ...
    If *tablename* is not given, it will be :file:`<track>_intervals` where track is derived from ``infile``.
