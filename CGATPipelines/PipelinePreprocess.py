...
class MasterProcessor(Mapping.SequenceCollectionProcessor):
    ...
    def build(self, infile, outfiles, track):
        ...
        outfiles : list
            list with three outfiles for SE and PE case, for example
            [test.fastq.gz, test.fastq.1.gz, test.fastq.2.gz]
        '''

        # preprocess to convert non-fastq into fastq
        # if already fastq, outfiles will be infiles
        cmd_preprocess, current_files = self.preprocess(infile, outfiles[0])

        # preprocess currently returns a list of lists
        current_files = current_files[0]
        nfiles = len(current_files)

        # hack for paired-end/single-end data
        if nfiles == 2:
            outfiles = outfiles[1:]
        else:
            outfiles = outfiles[0]
        ...
        for idx, processor in enumerate(self.processors):
            ...
            if idx == len(self.processors)-1:
                ...
                # add a prefix to each file denoting the processing step
                next_files = [processor.prefix + "-" + os.path.basename(x)
            ...
            cmd_process = processor.build(current_files,
                                          next_files,
                                          track)
...
class Trimgalore(ProcessTool):
    ...
    def build(self, infiles, outfiles, track):
        ...
        if len(infiles) == 1:
            infile = infiles[0]
            outfile = outfiles[0]
            trim_out = "%s_trimmed.fq.gz" % (track)
            ...
            cmd = '''trim_galore %(processing_options)s
                     ...
                     %(infile1)s %(infile2)s
                     2>>%(outfile)s.log;
                     mv %(track1)s_val_1.fq.gz %(outfile1)s;
                     mv %(track2)s_val_2.fq.gz %(outfile2)s;
                     ''' % locals()
...
class Trimmomatic(ProcessTool):
    ...
    def build(self, infiles, outfiles, track):
        ...
        if len(infiles) == 1:
            ...
            cmd = '''trimmomatic SE
            -threads %(threads)s -phred%(offset)s
            %(infile)s %(outfile)s
            %(processing_options)s 2>> %(outfile)s.log
            ;''' % locals()
            outfiles = (outfile,)
            ...
            -threads %(threads)s -phred%(offset)s
            %(infile1)s %(infile2)s
            %(outfile1)s %(track)s.1.unpaired
            %(outfile2)s %(track)s.2.unpaired
